...
from tokmint.settings import get_secconfig_subdir, get_unsafe_logging
...
async def mint_token(
    ...
    _unsafe = get_unsafe_logging()
    if _unsafe:
        logger.warning(
            "",
            extra={
                "event": "unsafe_logging_transaction",
                "phase": "begin",
                "path": "/v1/token",
            },
        )
    try:
        ...
        if logger.isEnabledFor(VERBOSE):
            _qp: dict[str, Any] = {"profile": prof, "domain": dom_raw}
            if cid is not None:
                _qp["client_id"] = cid
            if tid is not None:
                _qp["token_id"] = tid
            if kid is not None:
                _qp["key_id"] = kid
            if dkid is not None:
                _qp["dpop_key_id"] = dkid
            _htm = _query_value(dpop_htm)
            _htu = _query_value(dpop_htu)
            if _htm is not None:
                _qp["dpop_htm"] = _htm
            if _htu is not None:
                _qp["dpop_htu"] = _htu
        ...
        if cid is not None:
            ...
            if method == "private_key_jwt":
                ...
                if not isinstance(iss, str) or not iss.strip():
                    raise TokmintError(
                        400,
                        "PROFILE_INVALID",
                        "Invalid profile: private_key_jwt.assertion_iss "
                        "invalid.",
                    )
                ...
                priv = load_private_key_from_signing_key_row(
                    sk_row, seconfig_base
                )
                ...
                if kid is not None:
                    raise TokmintError(
                        400,
                        "INVALID_PARAMETER",
                        "key_id must be unset for client secret "
                        "authentication.",
                    )
            ...
            # When the caller supplied dpop_htm/dpop_htu and the upstream
            # issued a DPoP-bound token, build a proof for their actual API
            # request so Postman can attach it as the DPoP header without
            # extra round-trips.
            ...
            if logger.isEnabledFor(VERBOSE):
                _rb: dict[str, Any] = {
                    "access_token": (
                        str(out["access_token"]) if _unsafe else _REDACTED
                    ),
                    "token_type": out["token_type"],
                }
                if "expires_in" in out:
                    _rb["expires_in"] = out["expires_in"]
                if "dpop_proof" in out:
                    _rb["dpop_proof"] = (
                        out["dpop_proof"] if _unsafe else _REDACTED
                    )
                logger.log(
                    VERBOSE,
                    "",
        ...
        if logger.isEnabledFor(VERBOSE):
            logger.log(
                ...
                extra={
                    ...
                    "status": 200,
                    "response_body": {
                        "access_token": secret if _unsafe else _REDACTED,
